...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("tom://"))
    {
        uri.replace(0, 12, "tom:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "tom.lnk";
}
...
{
    return GetAutostartDir() / "tom.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("tom-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  tom-qt [" + tr("command-line options") + "]                     " + "\n";
